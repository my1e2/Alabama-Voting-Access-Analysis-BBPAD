# using osrm for network distance calculations
...
Calculates actual driving distances from population centers to polling places
"""
...
# osrm server expected to be running locally on port 5001
...
def get_osrm_distance(lon1, lat1, lon2, lat2):
    """
    get driving distance between two points using osrm routing engine.
    
    sends a request to the local osrm server for the shortest driving route
...
def calculate_network_distances(pop_centers_gdf, polling_gdf, sample_size=None):
    """
    calculate network driving distances from each population center to all polling places.
    
    uses the osrm routing engine to compute actual road network distances.
    processes each origin-destination pair individually with a small delay
    ...
    print(f"")
    print(f"Calculating network distances for {n_centers} centers x {n_polls} polling places")
    ...
    for i, (_, center) in enumerate(pop_centers_gdf.iterrows()):
        ...
        for j, (poll_lon, poll_lat) in enumerate(poll_coords):
            dist = get_osrm_distance(center_lon, center_lat, poll_lon, poll_lat)
    ...
    pop_centers_gdf['min_network_dist_miles'] = min_distances
    pop_centers_gdf['nearest_poll_network_idx'] = nearest_poll_idx
    ...
    if valid_count > 0:
        print(f"Average network distance: {np.nanmean(min_distances):.2f} miles")
        print(f"Maximum network distance: {np.nanmax(min_distances):.2f} miles")
        print(f"Minimum network distance: {np.nanmin(min_distances):.2f} miles")
...
def main():
    ...
    verifies osrm server connection, loads population centers and polling places,
    calculates network driving distances, and saves results to csv files.
    """
    print("Network distance calculations for Montgomery County")
    print("")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("Make sure you ran: osrm-routed alabama-latest.osrm --port 5001")
            return
    except requests.exceptions.ConnectionError:
        print("Cannot connect to OSRM server")
        print("Make sure you started the server on port 5001")
        return
    ...
    # calculate network distances for all centers
    # set sample_size=10 for a quick test, or none for full calculation
    pop_centers, distance_matrix = calculate_network_distances(
        pop_centers, 
        polling,
    ...
    # save accessibility scores with network distances included
    output_path = PROJECT_ROOT / "data" / "outputs" / "accessibility_scores_network.csv"
    pop_centers.to_csv(output_path, index=False)
    print(f"Saved to {output_path}")
    
    # save full distance matrix for detailed analysis
    matrix_path = PROJECT_ROOT / "data" / "outputs" / "distance_matrix_network.csv"
    ...
    if 'min_dist_to_poll_miles' in pop_centers.columns:
        valid_mask = ~np.isnan(pop_centers['min_network_dist_miles'])
        if valid_mask.sum() > 0:
            euclidean_avg = pop_centers.loc[valid_mask, 'min_dist_to_poll_miles'].mean()
            network_avg = pop_centers.loc[valid_mask, 'min_network_dist_miles'].mean()
            ratio = network_avg / euclidean_avg
            
            print(f"Average Euclidean distance: {euclidean_avg:.2f} miles")
            print(f"Average Network distance: {network_avg:.2f} miles")
            print(f"Network/Euclidean ratio: {ratio:.2f}x")
